...
# ── Load + preprocess (tweet_eval, max 1000 samples) ─────────────────────
df = load_data(max_samples=1000)
    ...
    for label_id in sorted(label_names.keys()):
        ...
        if count < 50:
            warnings.warn(
                ...
                f"samples (< 50). Model may underperform on this class."
            )
...
# ── Training args (optimized for CPU with small dataset) ──────────────────
training_args = TrainingArguments(
    output_dir="./results",
    num_train_epochs=5,                    # More epochs since dataset is small
    per_device_train_batch_size=8,         # Smaller batch for CPU memory
...
print("✅ DistilRoBERTa 3-class model saved to sentiment_model/")
